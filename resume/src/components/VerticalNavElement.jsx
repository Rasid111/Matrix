function VerticalNavElement({section, setSection, id, src}) {
    return (
        ...
        onMouseLeave={(ev) => {if(section === id){return}else ev.target.style.backgroundColor = ""; ev.target.querySelector("img").style.filter="brightness(1)"}} xs={2} style={{height: 75, width: 75, backgroundColor: section===id?"rgb(255,174,0)":""}} className="p-2 d-block rounded border">
